...
function HealBot_Aura_CheckCurDebuff(button)
    ...
    elseif HealBot_AuraDebuffCache[uaDebuffData[button.id][uaDebuffSlot].spellId]["typePrio"]>HealBot_AuraDebuffCache[uaDebuffData[button.id][uaDebuffSlot].spellId]["customPrio"] and HealBot_AuraDebuffCache[uaDebuffData[button.id][uaDebuffSlot].spellId]["customPrio"]<21 then
        ...
        ccdbCheckthis=false
        if HealBot_Aura_CanDispell[uaDebuffData[button.id][uaDebuffSlot].spellId] and HealBot_AuraDebuffCache[uaDebuffData[button.id][uaDebuffSlot].spellId]["typePrio"]<21 and 
           hbDebuffOnCD[uaDebuffData[button.id][uaDebuffSlot].debuffType]<HealBot_TimeNow and 
...
local function HealBot_Aura_PostUpdateUnitBuffsData(button, spellID, spellName)
    ...
    if not HealBot_AuraBuffCache[spellID] or HealBot_AuraBuffCache[spellID].reset then
        if not HealBot_AuraBuffCache[spellID] then HealBot_AuraBuffCache[spellID]={} end
        ...
        HealBot_AuraBuffXRef[HealBot_AuraBuffCache[spellID]["pid"]]=spellID
    ...
    if not HealBot_Aura_IconSet[spellID] or HealBot_AuraBuffCache[spellID].reset then
...
local function HealBot_Aura_PostUpdateUnitBuffsCurrent(button, spellID, spellName, count)
    HealBot_UnitBuffCurrent[button.guid][spellName]=true
    HealBot_Aura_BuffUpdate_Plugins(button, spellName, HealBot_Globals.CustomBuffTag[spellID] or HealBot_BuffTagNames[spellName], count, true, uaUnitCasterIsPlayer)
end
    ...
    if uaExpirationTime then
        ...
        if not HealBot_ExcludeBuffInCache[uaSpellId] and uaSlot<20 then
            ...
            if uaBuffData[button.id][uaSlot].spellId~=uaSpellId or uaBuffData[button.id][uaSlot].sourceUnit~=(uaUnitCaster or "nil") then
                ...
                uaBuffData[button.id][uaSlot].sourceUnitIsPlayer=uaUnitCasterIsPlayer
        ...
        HealBot_Aura_PostUpdateUnitBuffsCurrent(button, uaSpellId, uaName, uaCount)
    end
end
    ...
    if auraData.expirationTime then
        ...
        if not HealBot_ExcludeBuffInCache[auraData.spellId] and uaSlot<20 then
            ...
            if uaBuffData[uaID][uaSlot].spellId~=auraData.spellId or uaBuffData[uaID][uaSlot].sourceUnit~=(auraData.sourceUnit or "nil") then
                ...
                uaBuffData[uaID][uaSlot].sourceUnitIsPlayer=uaUnitCasterIsPlayer
        ...
        HealBot_Aura_PostUpdateUnitBuffsCurrent(HealBot_Buttons[uaID], auraData.spellId, auraData.name, auraData.applications)
    end
...
local function HealBot_Aura_PostUpdateUnitDebuffsData(button, spellID, spellName)
    ...
    if not HealBot_AuraDebuffCache[spellID] or HealBot_AuraDebuffCache[spellID].reset then
        ...
        if HealBot_AuraDebuffCache[spellID]["customPrio"]<HealBot_AuraDebuffCache[spellID]["typePrio"] then
            HealBot_AuraDebuffCache[spellID]["priority"]=HealBot_AuraDebuffCache[spellID]["customPrio"]
        else
            HealBot_AuraDebuffCache[spellID]["priority"]=HealBot_AuraDebuffCache[spellID]["typePrio"]
        end
        if HealBot_AuraDebuffCache[spellID]["priority"]<10 then
            HealBot_AuraDebuffCache[spellID]["pid"]="x0"..HealBot_AuraDebuffCache[spellID]["priority"]..spellID
        else
            HealBot_AuraDebuffCache[spellID]["pid"]="x"..HealBot_AuraDebuffCache[spellID]["priority"]..spellID
        end
        HealBot_AuraDebuffXRef[HealBot_AuraDebuffCache[spellID]["pid"]]=spellID
    ...
    if not HealBot_Aura_IconSet[spellID] or HealBot_AuraDebuffCache[spellID].reset then
...
local function HealBot_Aura_PostUpdateUnitDebuffsCurrent(button, spellID, spellName, count)
    HealBot_UnitDebuffCurrent[button.guid][spellName]=true
    HealBot_Aura_DebuffUpdate_Plugins(button, spellName, HealBot_Globals.CDCTag[spellID] or HealBot_DebuffTagNames[spellName], count, true, uaUnitCasterIsPlayer)
end
    ...
    if uaExpirationTime then
        ...
        if uaSlot<20 then
            ...
            if uaDebuffData[button.id][uaSlot].spellId~=uaSpellId or uaDebuffData[button.id][uaSlot].sourceUnit~=(uaUnitCaster or "nil") then
                ...
                uaDebuffData[button.id][uaSlot].sourceUnitIsPlayer=uaUnitCasterIsPlayer
        ...
        HealBot_Aura_PostUpdateUnitDebuffsCurrent(button, uaSpellId, uaName, uaCount)
    end
end
    ...
    if auraData.expirationTime then
        ...
        if uaSlot<20 then
            ...
            if uaDebuffData[uaID][uaSlot].spellId~=auraData.spellId or uaDebuffData[uaID][uaSlot].sourceUnit~=auraData.sourceUnit then
                ...
                uaDebuffData[uaID][uaSlot].sourceUnitIsPlayer=uaUnitCasterIsPlayer
        ...
        HealBot_Aura_PostUpdateUnitDebuffsCurrent(HealBot_Buttons[uaID], auraData.spellId, auraData.name, auraData.applications)
    end
end
...
function HealBot_Aura_UpdateActiveDebuffs()
    HealBot_Aura_UpdateAllIsCurrentDebuffs()
end
...
function HealBot_Aura_UpdateAllIsCurrentDebuffs()
    for _,xButton in pairs(HealBot_Unit_Button) do
