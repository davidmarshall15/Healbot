...
function HealBot_TalentQuery(button)
    ...
    if button.status.current<HealBot_Unit_Status["RESERVED"] then
        ...
        local s=" "
        if HealBot_Panel_RaidUnitButtonCheck(button.unit) then
...
local hbManaExtra={}
function HealBot_ManaExtra(unit, state)
      --HealBot_setCall("HealBot_ManaExtra", nil, guid)
    if state then
        hbManaExtra[unit]=true
    elseif not hbAuraWatchMana[unit] and not hbActionManaWatch[unit] then
        hbManaExtra[unit]=nil
    end
end

function HealBot_AuraWatchMana(unit, state)
      --HealBot_setCall("HealBot_AuraWatchMana", nil, guid)
    if state then
        hbAuraWatchMana[unit]=true
    else
        hbAuraWatchMana[unit]=nil
    end
    HealBot_ManaExtra(unit, state)
end

function HealBot_ActionWatchMana(unit, state)
      --HealBot_setCall("HealBot_ManaWatchClear", nil, guid)
    if state then
        hbActionManaWatch[unit]=true
    else
        hbActionManaWatch[unit]=nil
    end
    HealBot_ManaExtra(unit, state)
end
...
function HealBot_UnitMana(button)
    ...
    if button.status.current<HealBot_Unit_Status["DEAD"] then
        ...
        if HealBot_Util_isMidnight(false) or button.mana.current~=hbPowerCurrent or button.mana.max~=hbPowerMax then
            ...
            if hbManaExtra[button.unit] then
                if hbAuraWatchMana[button.unit] then
                    HealBot_Plugin_AuraWatch_ManaUpdate(button)
                end
                if hbActionManaWatch[button.unit] then
                    HealBot_ActionIcons_UpdateMana(button.unit, button.guid, button.mana.pct)
                end
            end
        ...
        if HealBot_Util_isMidnight(false) or button.poweralt.current~=hbPowerCurrent or button.poweralt.max~=hbPowerMax then
        ...
        if HealBot_Util_isMidnight(false) or button.mana.current>0 or button.mana.max>0 then
        ...
        if HealBot_Util_isMidnight(false) or button.poweralt.current>0 or button.poweralt.max>0 then
...
function HealBot_UpdateUnitGUIDChange(button, notRecalc)
    ...
    if button.status.classknown then
        ...
        HealBot_ClearGUIDQueue[button.id]=nil
        HealBot_UpdateUnitClear(button, true)
        if notRecalc then
...
function HealBot_UnitClass(button)
    ...
    if HealBot_Action_getGuidData(button.guid, "CLASSKNOWN") then
        ...
        ucUnitIsPlayer=UnitIsUnit(button.unit, "player")
        if HealBot_issecretvalue(ucUnitIsPlayer) then ucUnitIsPlayer=button.isplayer end
        if ucUnitIsPlayer then
            button.player=true
...
function HealBot_GUIDUnit(guid)
      --HealBot_setCall("HealBot_Update_ClearClassGuidData")
    if not HealBot_issecretvalue(guid) then 
        for xUnit,xButton in pairs(HealBot_Unit_Button) do
            if xButton.guid == guid then
                return xUnit
            end
        end
...
function HealBotClassic_HealsInUpdate(spellId, ...)
    ...
    for i=1, select("#", ...) do
        ...
        if chiTargetGUID then
            xUnit=HealBot_GUIDUnit(chiTargetGUID)
...
local hbHealthExtra={}
function HealBot_HealthExtra(unit, state)
      --HealBot_setCall("HealBot_HealthExtra", nil, guid)
    if state then
        hbHealthExtra[unit]=true
    elseif not hbAuraWatchHealth[unit] and not hbActionHealthWatch[unit] then
        hbHealthExtra[unit]=nil
    end
end

function HealBot_AuraWatchHealth(unit, state)
      --HealBot_setCall("HealBot_AuraWatchHealth")
    if state then
        hbAuraWatchHealth[unit]=true
    else
        hbAuraWatchHealth[unit]=nil
    end
    HealBot_HealthExtra(unit, state)
end

function HealBot_ActionWatchHealth(unit, state)
      --HealBot_setCall("HealBot_ActionWatchHealth", nil, guid)
    if state then
        hbActionHealthWatch[unit]=true
    else
        hbActionHealthWatch[unit]=nil
    end
    HealBot_HealthExtra(unit, state)
end
...
function HealBot_UnitHealth(button, force)
    ...
    if button.status.current<HealBot_Unit_Status["DC"] then
        ...
        if healthIsSecret or ((health~=button.health.current) or (healthMax~=button.health.max)) then
            ...
            if hbHealthExtra[button.unit] then
                if hbAuraWatchHealth[button.unit] then
                    HealBot_Plugin_AuraWatch_HealthUpdate(button)
                end
                if hbActionHealthWatch[button.unit] then
                    HealBot_ActionIcons_UpdateHealth(button.unit, button.health.pct)
                end
            end
...
function HealBot_CurrentPlayerHealth(unit)
    aButton=HealBot_Panel_RaidButton(unit)
    if aButton then
...
function HealBot_MaxPlayerHealth(unit)
    aButton=HealBot_Panel_RaidButton(unit)
    if aButton then
...
function HealBot_AfterCombatCleanup()
    ...
    if HealBot_luVars["RegenDisabled"] then
        ...
        for unit in pairs(HealBot_MobTarget) do
            HealBot_MobTarget[unit]=nil
        end
...
function HealBot_ActionWatchFalling(unit, state)
      --HealBot_setCall("HealBot_ActionWatchFalling", nil, guid)
    if state then
        hbActionFallWatch[unit]=true
    else
        hbActionFallWatch[unit]=nil
    end
end

function HealBot_ActionWatchSwimming(unit, state)
      --HealBot_setCall("HealBot_ActionWatchSwimming")
    if state then
        hbActionSwimWatch[unit]=true
    else
        hbActionSwimWatch[unit]=nil
    end
end
...
function HealBot_CheckFalling(button, cTime)
    ...
    if button.status.fallstart and button.status.fallstart == cTime then
        ...
        HealBot_ActionIcons_UpdateFalling(button.unit, button.guid, button.status.falling)
    end
end
    ...
    if HealBot_UnitExists(button) then
        if button.status.slowupdate then
            ...
            elseif button.status.postupdate then
                ...
                if hbActionFallWatch[button.unit] then
                    HealBot_ActionIcons_UpdateFalling(button.unit, button.guid, button.status.falling)
                end
                if hbActionSwimWatch[button.unit] then
                    HealBot_ActionIcons_UpdateSwimming(button.unit, button.guid, button.status.swimming)
                end
        ...
        elseif not HealBot_Data["UILOCK"] and button.specupdate>0 and button.specupdate<HealBot_TimeNow and HealBot_luVars["TalentQueryEnd"]<HealBot_TimeNow then
            if button.frame<10 and button.level>9 and button.isplayer then
                if HEALBOT_GAME_VERSION<5 and not HealBot_Panel_RaidUnitButtonCheck(button.unit) then
                    button.specupdate=0
                elseif HealBot_Panel_AllUnitButtonCheck(button.unit) then
                    if button.player then
        ...
        elseif button.mana.changetime and button.mana.changetime<HealBot_TimeNow then
            ...
            button.mana.change=true
            HealBot_UnitMana(button)
        elseif button.frame<10 and not HealBot_issecretvalue(button.guid) then
            if button.status.castend>0 and button.status.castend<(HealBot_TimeNow*1000) then
                HealBot_Aux_ClearCastBar(button)
            elseif hbActionSwimWatch[button.unit] and button.status.swimming~=IsSwimming(button.unit) then
...
function HealBot_PluginClearDown()
    if HealBot_luVars["pluginClearDown"]>0 then
        ...
        elseif HealBot_luVars["pluginClearDown"]<6 and HealBot_luVars["pluginTimeToLive"] then
            HealBot_Plugin_TimeToLive_Cleardown()
            HealBot_luVars["pluginClearDown"]=6
        elseif HealBot_luVars["pluginClearDown"]<7 and HealBot_luVars["pluginTimeToDie"] then
...
function HealBot_SetDupIDs()
    for _,pButton in pairs(HealBot_Private_Button) do
        if pButton.status.duplicate then
            xButton=HealBot_Panel_RaidUnitDupButton(pButton.unit)
            if xButton then
    ...
    for _,xButton in pairs(HealBot_Unit_Button) do
        if xButton.status.duplicate then
            pButton=HealBot_Panel_RaidUnitPrivDupButton(xButton.unit)
            if pButton then
                pButton.status.dupid=xButton.id
                xButton.status.dupid=pButton.id
            end
...
function HealBot_UnitUpdateButton(button)
    ...
    if HealBot_UnitExists(button) then
        ...
        elseif button.status.refresh then
            ...
            if hbActionFallWatch[button.unit] and button.status.falling~=IsFalling(button.unit) then
                button.status.falling=IsFalling(button.unit)
...
function HealBot_Update_Final()
    ...
    if not HealBot_luVars["ProcessRefresh"] then
        for id,guid in pairs(HealBot_ClearGUIDQueue) do
            HealBot_ClearGUID(guid)
            HealBot_ClearGUIDQueue[id]=nil
            break
        end
    end
    ...
    elseif (HealBot_Data["UILOCK"] or not HealBot_luVars["UpdateEnemyFrame"]) and HealBot_luVars["AllOutOfCombatCheck"]<=HealBot_TimeNow then
        ...
        for xUnit,xButton in pairs(HealBot_Private_Button) do
            ...
            if xButton.status.current<HealBot_Unit_Status["DEAD"] and xButton.range.current>-1 and UnitAffectingCombat(xUnit) and
               HealBot_ValidLivingEnemy(xUnit, ouEnemyUnit) and (HealBot_Util_isMidnight(false) or UnitIsUnit(xUnit, ouEnemyTarget)) then
...
function HealBot_Update_ResetRefreshLists()
    ...
    if HealBot_luVars["auraWatchIncEnemy"] then
        for unit,_ in pairs(hbPrevGUIDs) do
            if not HealBot_Panel_AllUnitButtonCheck(unit) then
                HealBot_luVars["pluginClearDown"]=1
                HealBot_Action_UpdateGlobalDimming(0)
                hbPrevGUIDs[unit]=nil
            end
        end
        for _,xButton in pairs(HealBot_Enemy_Button) do
        ...
        for unit,_ in pairs(hbPrevGUIDs) do
            if not HealBot_Panel_RaidUnitButtonCheck(unit) then
                HealBot_luVars["pluginClearDown"]=1
                hbPrevGUIDs[unit]=nil
            end
        end
...
function HealBot_UnitEnemyTargetUpdate(unit, guid)
    if HealBot_MobTarget[unit] and UnitExists(unit.."target") then
        aButton=HealBot_Panel_AllButton(unit.."target")
        if aButton and not aButton.special.unit and hbv_Skins_GetFrameVar("BarAggro", "TARGETED", aButton.frame) then
            if not hbv_Skins_GetFrameVar("BarAggro", "TARBOSSONLY", aButton.frame) or (HealBot_UnitClassification(unit) or "") == "Boss" then
                if HealBot_MobTarget[unit] ~= aButton.unit then
                    if HealBot_Panel_AllUnitButtonCheck(unit) then
                        HealBot_luVars["NoLongerTargets"]=true
                        for eUnit, pUnit in pairs(HealBot_MobTarget) do
                            if eUnit ~= unit and pUnit == HealBot_MobTarget[unit] then
                                HealBot_luVars["NoLongerTargets"]=false
                                break
                            end
                        end
                        if HealBot_luVars["NoLongerTargets"] then
                            HealBot_Aggro_PlayerNotTargeted(HealBot_MobTarget[unit])
                        end
                    end
                    HealBot_MobTarget[unit]=aButton.unit
...
function HealBot_CalcThreat(button)
    ...
    if button.status.current<HealBot_Unit_Status["DEAD"] and UnitIsFriend("player",button.unit) then
        ...
        if not HealBot_Util_isMidnight(false) and ctEnemyUnit then
...
function HealBot_SpecChange(button)
    ...
    if button.player then
        ...
        if HealBot_Panel_RaidUnitButtonCheck(button.unit) then HealBot_Action_SetGuidData(button, "TMPSPEC", " ") end
        HealBot_SpecUpdate(button, HealBot_TimeNow)
    end
end
...
function HealBot_SetTargetBar()
    ...
    if UnitExists("target") then
        xButton, pButton=HealBot_Panel_AllUnitButton(UnitGUID("target"), "target")
...
function HealBot_UnitPet(unit)
    ...
    elseif HealBot_SafeUnitIsUnit(unit,"player") then
        upUnit="pet"
...
function HealBot_ReadyCheckUnit(button)
    ...
    if HealBot_luVars["rcEnd"] and button.isplayer and HealBot_Panel_RaidUnitButtonCheck(button.unit) and HealBot_luVars["rcEnd"]>HealBot_TimeNow then
...
function HealBot_ClearGUID(guid)
    ...
    if guid and not HealBot_Panel_AllUnitButtonCheck(guid, "x") then
